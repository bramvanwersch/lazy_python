...
def _talk(*args):
    # TODO: add tests
    # activities are allowed to continue when talking
    active_user_dir = lazy_utility.active_user_dir()
    current_area, curren_location = lazy_utility.get_values_from_file(
        active_user_dir / lazy_constants.USER_GENERAL_FILE_NAME, [lazy_constants.USERFILE_GENERAL_CURRENT_AREA,
                                                                  lazy_constants.USERFILE_GENERAL_CURRENT_LOCATION])
    if current_area == "":
        lazy_warnings.warn(lazy_warnings.LazyWarningMessages.UNSELECTED_AREA)
        return
    if curren_location == "":
        lazy_warnings.warn(lazy_warnings.LazyWarningMessages.UNSELECTED_LOCATION)
        return
    area_obj = areas.AREA_MAPPING[current_area]
    location_obj = area_obj.locations[curren_location]
    people_dict = location_obj.people
    people_names = list(people_dict.keys())
    if len(people_names) == 0:
        lazy_utility.message("No people at this location.")
        return
    if len(args) == 0:
        lazy_utility.message_question(f"Please choose one of the following people: "
                                      f"{', '.join(people_names)}")
        selected_value = lazy_utility.ask_answer(f"Please choose one of: {', '.join(people_names)}",
                                                 {name: name for name in people_names}, False)
    else:
        selected_value = args[0]
        if selected_value not in people_dict:
            lazy_warnings.warn(lazy_warnings.LazyWarningMessages.INVALID_PERSON, name=selected_value,
                               people=', '.join(people_names), location=location_obj.name)
            return
    person_object = people_dict[selected_value]
    person_object.talk()
